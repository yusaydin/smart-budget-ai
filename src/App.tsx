...
import { extractExpenseFromImage, generateMonthlyReport, getCorporateAdvice } from './services/gemini';
    ...
    <div className="min-h-screen bg-[#050507] text-slate-200 font-sans selection:bg-violet-500/30 pb-24">
      ...
      <main className="px-6 max-w-2xl mx-auto py-6">
        <AnimatePresence mode="wait">
          {activeTab === 'dashboard' && <Dashboard profile={profile} expenses={expenses} key="dashboard" />}
          {activeTab === 'expenses' && <ExpenseListView expenses={expenses} key="expenses" />}
...
function Dashboard({ profile, expenses }: { profile: UserProfile | null, expenses: Expense[] }) {
  ...
  const pieData = {
    ...
    datasets: [{
      ...
      backgroundColor: [
        '#2563eb', '#3b82f6', '#60a5fa', '#93c5fd', '#bfdbfe', '#1e40af'
      ],
      borderWidth: 0,
    }]
  };
...
function ExpenseListView({ expenses }: { expenses: Expense[] }) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-white/5 rounded-xl p-1">
          <button className="px-4 py-2 bg-white/10 rounded-lg text-xs font-medium">All</button>
          <button className="px-4 py-2 text-xs font-medium text-gray-500">Personal</button>
          <button className="px-4 py-2 text-xs font-medium text-gray-500">Work</button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {expenses.length === 0 && (
          <div className="text-center py-20 bg-white/5 rounded-[2rem] border border-dashed border-white/10">
            ...
            <p className="text-gray-400">Your expense list is empty.</p>
          </div>
        )}
        {expenses.map(exp => (
    ...
    <div className="p-2 transition-all group border-b border-white/5 last:border-0 rounded-lg hover:bg-white/5">
      <div className="flex gap-3 items-center justify-between">
        <div className="flex gap-3 items-center">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-200 truncate max-w-[120px] sm:max-w-xs">{expense.merchant}</h4>
        ...
        <div className="text-right flex flex-col items-end">
          ...
          {expense.isCorporate && <div className="mt-1 px-1.5 py-0.5 rounded bg-violet-500/20 text-[8px] font-bold text-violet-400 uppercase">Corp</div>}
        </div>
      </div>

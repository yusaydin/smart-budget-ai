...
const newSettingsView = `function SettingsView({
  ...
  const handleEditName = async () => {
    const newName = prompt("Tam adınızı girin:", profile?.displayName || "");
    if (newName && newName !== profile?.displayName && auth.currentUser) {
      await updateDoc(doc(db, "users", auth.currentUser.uid), { displayName: newName });
    }
  };

  const handleEditPhone = async () => {
    const newPhone = prompt("Telefon numaranızı girin:", profile?.phone || "+90 555 123 4567");
    if (newPhone && newPhone !== profile?.phone && auth.currentUser) {
        ...
        <section className="bg-white dark:bg-surface-container rounded-[12px] p-5 shadow-[0px_2px_8px_rgba(0,0,0,0.04)] border border-slate-100/80 dark:border-surface-variant">
          ...
          <div className="space-y-0">
            <div 
              ...
              className="flex justify-between items-center pb-[14px] mb-[14px] border-b border-slate-100/80 dark:border-surface-variant cursor-pointer hover:opacity-70 active:scale-[0.98] transition-all"
            >
              <div>
            ...
            <div className="flex justify-between items-center pb-[14px] mb-[14px] border-b border-slate-100/80 dark:border-surface-variant cursor-pointer hover:opacity-70 active:scale-[0.98] transition-all" onClick={() => alert("E-posta adresi değiştirilemez.")}>
              <div>
                <p className="text-[12px] text-slate-800 dark:text-on-surface mb-[2px] font-normal">
                  Email
                </p>
...
console.log("Written updated SettingsView again!");

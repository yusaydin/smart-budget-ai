...
const newSettingsView = `function SettingsView({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0 }}
      className="space-y-lg pt-4 w-full"
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-lg pb-10">
        ...
        <div className="space-y-lg">
          ...
          <section className="bg-surface-container-lowest rounded-xl p-lg shadow-[0px_4px_12px_rgba(49,124,184,0.08)]">
            <h3 className="font-label-md text-label-md text-primary mb-md uppercase tracking-wider">
              Kişisel Bilgiler
            </h3>
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center py-2 border-b border-surface-variant last:border-0">
                <div>
                  <p className="font-label-md text-label-md text-on-surface-variant">E-posta</p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {COMMON_CURRENCIES.map((c) => (
                        <option className="text-on-surface" key={c} value={c}>
                          {c}
                        </option>
                      ))}
                    </select>
          ...
          <section className="bg-surface-container-lowest rounded-xl p-lg shadow-[0px_4px_12px_rgba(49,124,184,0.08)]">
            <h3 className="font-label-md text-label-md text-primary mb-md uppercase tracking-wider">
              Kategoriler & Bütçeler
            </h3>
            <div className="space-y-3 mb-6">
              {(profile?.categories || DEFAULT_CATEGORIES).map((cat, i) => (
                <div
                  key={i}
                  className="flex flex-col gap-2 w-full border-b border-surface-variant pb-3 last:border-0"
                >
            ...
            <form
              onSubmit={(e) => {
                ...
                if (
                  val &&
                  !(profile?.categories || DEFAULT_CATEGORIES).includes(val) &&
                  profile
                ) {
                  const newCats = [
                    ...(profile.categories || DEFAULT_CATEGORIES),
            ...
            >
              ...
              <button
                type="submit"
                className="material-symbols-outlined text-primary p-2 hover:bg-surface-container-high rounded-md transition-colors bg-surface-container"
              >
                add
              </button>
            </form>
        ...
        <div className="space-y-lg">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-2 border-b border-surface-variant">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-body-md text-body-md text-on-surface">Karanlık Mod</span>
                </div>
                <button className="w-12 h-6 bg-surface-variant rounded-full relative transition-colors focus:outline-none opacity-60 cursor-default">
                  <div className="w-5 h-5 bg-on-surface rounded-full absolute left-0.5 top-0.5 shadow-sm transition-transform"></div>
                </button>
              </div>
              ...
              <div className="flex justify-between items-center py-2 border-b border-surface-variant">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-body-md text-body-md text-on-surface">Anlık Bildirimler</span>
                </div>
                <button className="w-12 h-6 bg-primary rounded-full relative transition-colors focus:outline-none opacity-60 cursor-default">
                  <div className="w-5 h-5 bg-on-primary rounded-full absolute right-0.5 top-0.5 shadow-sm transition-transform"></div>
                </button>
              </div>
               ...
               <div className="flex justify-between items-center py-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-body-md text-body-md text-on-surface block">Kurumsal Profil</span>
                  </div>
                </div>
                <button
                  onClick={toggleCorporate}
                  className={\`w-12 h-6 rounded-full relative transition-colors focus:outline-none \${
                    profile?.isCorporate ? "bg-primary" : "bg-surface-variant"
                  }\`}
                >
                  <div
                    className={\`w-5 h-5 rounded-full absolute top-0.5 shadow-sm transition-transform \${
                      profile?.isCorporate
                        ? "bg-on-primary right-0.5 translate-x-0"
                        : "bg-on-surface left-0.5 translate-x-0"
                    }\`}
                  ></div>
                </button>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option className="text-on-surface bg-surface" value="3months">Son 3 Ay</option>
                    <option className="text-on-surface bg-surface" value="6months">Son 6 Ay</option>
                    <option className="text-on-surface bg-surface" value="manual">Sadece Manuel</option>
                  </select>
                  <span className="material-symbols-outlined text-[20px] text-on-surface-variant absolute right-2 top-2 pointer-events-none">
                    expand_more
                  </span>
                </div>
              </div>
                ...
                <input
                  ...
                  onBlur={async (e) => {
                    if (profile)
                      await setDoc(doc(db, "users", profile.uid), {
                        ...profile,
                        syncLabels: e.target.value.trim(),
                      });
                  }}
          ...
          <section className="bg-surface-container-lowest rounded-xl p-lg shadow-[0px_4px_12px_rgba(49,124,184,0.08)]">
            ...
            <div className="space-y-2">
              <button className="w-full flex justify-between items-center py-3 border-b border-surface-variant last:border-0 hover:bg-surface-container-low transition-colors rounded-md px-2 -mx-2">
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-on-surface-variant">shield</span>
                  <span className="font-body-md text-body-md text-on-surface">Veri Gizliliği</span>
                </div>
              ...
              <button
                onClick={async () => {
                  if (!profile) return;
                  if (!window.confirm("E-postalardan aktarılan tüm harcama kayıtlarını silmek istiyor musunuz?")) return;
                  try {
                    const q = query(
                    ...
                    alert("E-posta kaynaklı harcamalar silindi.");
                  } catch (err) {
                    handleFirestoreError(err, OperationType.DELETE, "expenses");
                  }
                }}
                className="w-full flex justify-between items-center py-3 border-b border-surface-variant last:border-0 hover:bg-surface-container-low transition-colors rounded-md px-2 -mx-2"
              >
          ...
          <section className="bg-surface-container-lowest rounded-xl p-lg shadow-[0px_4px_12px_rgba(49,124,184,0.08)]">
             ...
             <div className="space-y-2 mb-lg">
                ...
                <button className="w-full flex justify-between items-center py-3 hover:bg-surface-container-low transition-colors rounded-md px-2 -mx-2">
                  <div className="flex items-center gap-3">
                    <span className="material-symbols-outlined text-on-surface-variant">description</span>
                    <span className="font-body-md text-body-md text-on-surface">Kullanım Koşulları</span>
                  </div>
                  <span className="material-symbols-outlined text-on-surface-variant">chevron_right</span>
                </button>
             </div>

...
if (modalStart === -1) process.exit(1);

let newCode = code.substring(0, modalStart) + `function AddExpenseModal({ onClose, userId, profile, isCorporateDefault, categories }: { onClose: () => void, userId: string, profile: UserProfile | null, isCorporateDefault: boolean, categories: string[] }) {
    ...
    if (track) {
      try {
        ...
        if (caps.torch) {
          ...
          // Silently fail or ignore if flash unsupported
        }
      } catch (err) {
        console.error(err);
      }
